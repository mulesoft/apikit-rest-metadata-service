...
public class AmfHandler implements MetadataResolverFactory {
  ...
  public AmfHandler(ResourceLoader resourceLoader, Notifier notifier, boolean withFallBack) {
    this.resourceLoader = resourceLoader;
    this.notifier = notifier;
  ...
  Optional<WebApi> getApi(final String apiDefinition) {
    ...
    try {
      ...
      if (!report.conforms()) {
        if (withFallBack) {
          notifier.warn(format("Fail reading API definition '%s' using AMF parser.", apiDefinition));
        } else {
          report.getResults().forEach(error -> notifier.error(error.getMessage()));
        }
        return empty();
      }
